        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    $('.container__posts .grid').html('<p>Erro ao carregar posts. Tente novamente.</p>');
                }
            },
            error: function () {
                $('.container__posts .grid').html('<p>Erro ao processar a requisição.</p>');
            }
        });
    }
...
function createLoaderCard() {
    return `
        <div role="status" class="w-[calc(100%-0.2rem)] md:w-[calc(100%-0.4rem)] lg:w-[calc(100%-1rem)] mt-4 p-2 border border-gray-200 rounded-[20px] shadow animate-pulse bg-blackGray">
